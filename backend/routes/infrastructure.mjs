...
export default function createInfrastructureRoutes({ 
  ...
  // 🛡️ [SECURITY EXPORT ENDPOINT] (v2.6.370)
  ...
  // 🛡️ [UNIFIED SLACK GATEWAY] (v2.6.370)
  ...
  async function handleSlackUnified(req, res) {
    try {
      // 1. Detect if this is an Interaction (Button Click)
      if (req.body.payload) {
        return await handleInteract(req, res);
      }

      // 2. Detect if this is a Slash Command
      ...
      console.error("❌ Unified Slack Gateway Error:", err.message);
  ...
  async function handleInteract(req, res) {
    try {
      ...
      if (actionId === 'view_security_details') {
         ...
         // 🔐 [SECURITY ENFORCEMENT]: Generate a short-lived forensic token (v2.6.370)
         const jwt = await import('jsonwebtoken');
      ...
      // 🛡️ [CATCH-ALL RESPONDER] (v2.6.370)
      return res.json({
        replace_original: false,
        response_type: "ephemeral",
