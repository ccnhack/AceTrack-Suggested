...
export default function createInfrastructureRoutes({ 
  ...
  // 🛡️ [SECURITY EXPORT ENDPOINT] (v2.6.372)
  ...
  // 🛡️ [UNIFIED SLACK GATEWAY] (v2.6.372)
  ...
  async function handleInteract(req, res, isDelayed = false) {
    try {
      ...
      if (actionId === 'view_security_details') {
         ...
         // 🔐 [SECURITY ENFORCEMENT]: Generate a short-lived forensic token (v2.6.372)
         const jwt = await import('jsonwebtoken');
         ...
         if (isDelayed) {
            // First, acknowledge the click with a pulse
            await sendDelayedSlackResponse(responseUrl, { 
              replace_original: false, 
              response_type: "ephemeral", 
              text: "🔄 *Preparing secure forensic audit...* Please wait a few seconds." 
            });
      ...
      // 🛡️ [CATCH-ALL RESPONDER] (v2.6.372)
      const catchRes = {
        replace_original: false,
        response_type: "ephemeral",
