            ...
            try {
               ...
               const compactPlayers = players.map(p => {
                  ...
                  return {
                     ...
                     text: `[Database][Player Record] ID:${p.id} Name:${pd.name || pd.firstName || 'N/A'} Role:${p.role || pd.role || 'N/A'} Designation:${pd.designation || 'N/A'} Status:${pd.supportStatus || pd.status || 'active'} Email:${pd.email || 'N/A'} Phone:${pd.phoneNumber || pd.phone || 'N/A'} ShortLeaves:${pd.shortLeaves ? JSON.stringify(pd.shortLeaves) : 'None'}`
                  };
               });
            ...
            try {
               ...
               if (playerDoc) {
                  ...
                  combinedLogsArr.push({
                     ...
                     text: `[Database][Fallback Record] System found an active database profile matching "${userSearchTerm}". Name: ${profileName}. Username: ${profileUsername}. Email: ${profileEmail}. Phone: ${profilePhone}. Role: ${profileRole}. Designation: ${profileDesignation}. Original Onboard/Creation Time: ${istDate}. Last Data Update Time: ${istLastUpdated}.`
                  });
               }
            } catch(e) {
...
DO NOT wrap the JSON in markdown code blocks. Output ONLY valid, parsable JSON. No explanations.`;
         ...
         if (hasPlayerFilter) {
            ...
            try {
               ...
               const compactPlayers = players.map(p => {
                  const pd = p.data || {};
                  return `[Database][Player Record] ID:${p.id} Name:${pd.name || pd.firstName || 'N/A'} Role:${p.role || pd.role || 'N/A'} Designation:${pd.designation || 'N/A'} Status:${pd.supportStatus || pd.status || 'active'} Email:${pd.email || 'N/A'} Phone:${pd.phoneNumber || pd.phone || 'N/A'} ShortLeaves:${pd.shortLeaves ? JSON.stringify(pd.shortLeaves) : 'None'}`;
               });
               combinedLogsArr.push(...compactPlayers);
            } catch (err) {
