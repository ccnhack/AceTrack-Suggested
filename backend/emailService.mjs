// ═══════════════════════════════════════════════════════════════
// 📧 AceTrack Email Service (v2.6.124)
 ...
 */
export function buildOnboardingHtml(employeeName, email, setupLink, expiryFormatted) {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8">
 ...
 */
export async function sendOnboardingEmail(toEmail, setupLink, expiresAt) {
  ...
  try {
    ...
    console.log(`📧 Onboarding email sent to ${toEmail}: ${info.messageId}`);
