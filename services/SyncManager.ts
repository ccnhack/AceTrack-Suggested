...
class SyncManager {
  ...
  public async init(userId: string) {
    ...
    this.initPromise = (async () => {
      ...
      // 🛡️ [SYNC RECOVERY] (v2.6.124) Reset sync count and clear hung states
      this.activeSyncs = 0;
    ...
    return this.trackOperation(`SYNC_${Object.keys(updates).join('_')}`, async () => {
      ...
      for (const key in updates) {
        ...
        if (key === 'players' && Array.isArray(val)) {
          ...
          // 🛡️ [ADMIN BADGE INJECTION] (v2.6.124)
        ...
        // 🛡️ [TOURNAMENT SANITIZATION] (v2.6.124)
        ...
        // 🛡️ [TICKET MESSAGE STATE: DELIVERED] (v2.6.124)
  ...
  /**
   * 🛡️ SELF-HEALING CONFLICT RESOLUTION (v2.6.124)
  ...
  public async performCloudPush(isInternal: boolean = false): Promise<void> {
    // 🛡️ [FIX v2.6.124] Allow nested pushes from within tracked operations.
  ...
  private async pushToApi(updates: Record<string, any>, isInternal: boolean): Promise<{ success: boolean, status?: number }> {
    ...
    try {
      // 🛡️ [TICKET MESSAGE STATE: SENT] (v2.6.124)
      ...
      if (!response.ok) {
        if (response.status === 429) {
          ...
          // 🛡️ [OCC CONFLICT HANDLING] (v2.6.124)
